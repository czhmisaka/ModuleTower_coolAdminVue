 ...
 * @LastEditors: CZH
 * @LastEditTime: 2023-05-28 11:12:44
 * @FilePath: /cool-admin-vue/src/modules/moduleTower/components/grid/module/cardApi/index.ts
 */
 ...
 * @param {object} value
 */
export const checkContext = (content: { [key: string]: any }, value: { [key: string]: any }) => {
 ...
 * @Date: 2022-12-05 19:32:21
 */
export const hightLightComponent = (content: { [key: string]: any }, value: string[] = []) => {
	if (!checkContext(content, { 1: 1 })) return;
	try {
		let func = content["$emit"] ? "$emit" : "emit";
 ...
 * @Date: 2022-12-05 19:32:21
 */
export const addGridCell = (content: { [key: string]: any }, value: gridCellTemplate) => {
	if (!checkContext(content, value)) return;
	try {
		let func = content["$emit"] ? "$emit" : "emit";
		content[func]("onChange", value, {
 ...
 * @Date: 2022-12-05 19:32:21
 */
export const delGridCell = (content: { [key: string]: any }, value: string[]) => {
	if (!checkContext(content, value)) return;
	try {
		let func = content["$emit"] ? "$emit" : "emit";
		content[func]("onChange", value, {
